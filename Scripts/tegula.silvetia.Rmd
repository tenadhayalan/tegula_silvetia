...
carb_summary <- carb_summary %>%
  ...
  ungroup() %>%
  select(tank, experiment, ends_with("_formatted"))
...
temp_avg <- temp %>%
  ...
  summarize(
    avg_temp = mean(sea_water_temperature, na.rm = TRUE)  # calculate mean temp
...
tegsizefield %>%
  ggplot(aes(x=shell_length)) + 
    geom_histogram(position="dodge",bins=7,color="white", fill="peachpuff3")+
    theme_bw() + 
    theme(axis.text=element_text(size=15),axis.title=element_text(size=18), title = element_text(size=15))+
    labs(x="Shell length (mm)", y="Observations")

# shell width
tegshellwidthplot <-
tegsizefield %>%
  ggplot(aes(x=shell_width)) + 
    geom_histogram(position="dodge",bins=7,color="white", fill="peachpuff3")+
    theme_bw() + 
    theme(axis.text=element_text(size=15),axis.title=element_text(size=18), title = element_text(size=15))+
...
ggsave("tegula field.png", path=here("Output"), width = 10, height = 6) # save to output folder
...
rwmodelGP_bR_full <- lmer(GrossPhoto ~ pH * temp_treatment + (1 | tank/predator), 
                    data = rwrespo_clean %>% filter(experiment == "b"))
rwmodelGP_bR_reduced <- lmer(GrossPhoto ~ pH * temp_treatment + (1 | tank), 
                      data = rwrespo_clean %>% filter(experiment == "b"))
...
```{r eval=FALSE, echo=FALSE}
# start with april, do we see differences with predation?
rwmodelR_aR_full <- lmer(Respiration ~ pH * temp_treatment + (1 | tank/predator), 
                    data = rwrespo_clean %>% filter(experiment == "a"))
rwmodelR_aR_reduced <- lmer(Respiration ~ pH * temp_treatment + (1 | tank), 
                      data = rwrespo_clean %>% filter(experiment == "a"))
...
# what about may?
rwmodelR_bR_full <- lmer(Respiration ~ pH * temp_treatment + (1 | tank/predator), 
                    data = rwrespo_clean %>% filter(experiment == "b"))
rwmodelR_bR_reduced <- lmer(Respiration ~ pH * temp_treatment + (1 | tank), 
                      data = rwrespo_clean %>% filter(experiment == "b"))
