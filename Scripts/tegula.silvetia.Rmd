...
anova(rwmodelGP_b) #temp:predator
summary(rwmodelGP_b)
...
## take averages for control and predation cages
...
rwpredplot <-
rwrespo_clean %>%
  ggplot(aes(x = pH, y = GrossPhoto,
      color = as.factor(temp_treatment),
      fill  = as.factor(temp_treatment))) +
  facet_grid(predator ~ experiment, drop = TRUE, 
             labeller = as_labeller(c("a" = "Early spring", "b" = "Late spring", 
                                      "c" = "Control", "p" = "Predator"))) +
...
ggsave("rw predator response.png", path=here("Output"), width = 6.5, height = 5)
```


### Predation effect size plot?
```{r}
...
### GP effect size plot with predation
```{r}
rwrespo_avg_p <- rwrespo_clean %>%
  group_by(pH, temp_treatment, experiment, predator) %>%  # group for predator vs control per tank
  summarize(
...
rwmodelRP_a <- lm(Respiration ~ pH * temp_treatment * predator, 
              data = rwrespo_clean %>% filter(experiment == "a"))
anova(rwmodelRP_a) #model stats, pH x temp
summary(rwmodelRP_a)
...
avg_respo_model_april <- lm(avg_respo ~ pH * temp_treatment * predator, data = rwrespo_avg %>% filter(experiment == "a"))
#plot(avg_respo_model_april)
anova(avg_respo_model_april)
...
### Respo effect sizes with predation
```{r}
rwrespo_avg_p <- rwrespo_clean %>%
  group_by(pH, temp_treatment, experiment, predator) %>%  # group for predator vs control per tank
  summarize(
...
```{r}
# literature suggests an oxycaloric equivalent of roughly 0.460 J/ umol O2
tegenergy <- tegrespodry %>%
  mutate(teg_energy = Respiration * 0.457) #from Hill, Gilman papers

rwenergy <- rwrespo %>%
  mutate(rw_energy = GrossPhoto * 0.455) %>% #from Mascow et al 2019
  mutate(temp_treatment = as.factor(temp_treatment))

energy <- bind_rows(rwenergy, tegenergy) 

energy <- energy %>%
  group_by(experiment, pH, temp_treatment) %>%
  mutate(ratio = rw_energy/teg_energy)

tegenergy <- tegrespodry %>%
  mutate(energy = Respiration * 0.457) %>% #from Hill, Gilman papers
  group_by(temp_treatment, pH, experiment) %>%
  summarize(avg_teg_energy = mean(energy))

rwenergy <- rwrespo %>%
  mutate(energy = NetPhoto * 0.455) %>% #from Mascow et al 2019 #gibbs E is 482.9 from al najjar
  mutate(temp_treatment = as.factor(temp_treatment)) %>%
  group_by(temp_treatment, pH, experiment) %>%
  summarize(avg_rw_energy = mean(energy))

energy_avg <- left_join(rwenergy, tegenergy)

# expressed as ratios of energy gain to energy lost at each temp

energy_avg <- energy_avg %>%
  group_by(experiment, pH, temp_treatment) %>%
  mutate(ratio = avg_rw_energy/avg_teg_energy)

energymodel <- lm(ratio ~ pH * temp_treatment, data = energy_avg)
anova(energymodel)

poly_model <- lm(ratio ~ poly(pH, 2) * temp_treatment, data = energy_avg)
anova(poly_model)
plot(poly_model)
anova(energymodel, poly_model)
AIC(energymodel, poly_model)

# predict using model
energy <- energy %>%
  ungroup() %>%
  mutate(predicted_ratio = predict(poly_model, newdata = energy))

predictions <- predict(poly_model, newdata = energy, interval = "confidence")

# Add predictions and confidence intervals to calc_new
energy <- energy %>%
  mutate(predicted_ratio = predictions[, "fit"],
         lower_ci = predictions[, "lwr"],  # Lower confidence bound
         upper_ci = predictions[, "upr"])  # Upper confidence bound

# avg plot
energy %>%
  ggplot(aes(x = pH, y = ratio, color = temp_treatment, fill = temp_treatment)) +
  facet_wrap(~experiment,labeller = as_labeller(c("a" = "Early spring", "b" = "Late spring"))) +
  geom_point(data = energy_avg, aes(y = ratio), alpha = 1) +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = temp_treatment), 
              alpha = 0.3, color = NA) + # confidence interval
  geom_line(aes(y = predicted_ratio), size = 1.2) + # add model line
  scale_color_manual(values = c("skyblue2", "firebrick1")) +
  scale_fill_manual(values = c("skyblue2", "firebrick1")) +
  labs(y = "Theoretical ratio of energy gained:energy lost", x = "pH", 
       color = expression(paste("Temperature (", degree, "C)")),
       fill = expression(paste("Temperature (", degree, "C)"))) +
  theme_minimal()
```

### activation energy - equation 3 ahhh scary
```{r}
## Early Spring 

# Calculating Ea for herbivore

# Two temperatures
T1 <- 289.15  # 16°C in K
T2 <- 293.15  # 20°C in K

# calculate metabolic means
tegrespodryscale_mean <- tegrespodryscale %>%
  filter(experiment == "a") %>%
  group_by(temp_treatment) %>%
  summarize(mean_Respo = mean(Respo, na.rm = TRUE))
R1 <- 1.31380 # metabolic rate at T1
R2 <- 1.63047 # metabolic rate at T2

k <- 8.617e-5
EH <- - k * (log(R2/R1)) / (1/T2 - 1/T1)
EH  # 0.3943225 activation energy in eV

# Calculating Ea for rockweed
rwresposcale <- rwrespo %>%
  mutate(GP = umol.sec.corr * 3600) %>% #respo in umol/hr
  filter(experiment == "a") %>%
  group_by(temp_treatment) %>%
  summarize(mean_GP = mean(GrossPhoto, na.rm = TRUE))
R1 <- 56.30507
R2 <- 57.36463

EP <- - k * (log(R2/R1)) / (1/T2 - 1/T1)
EP # 0.03404338

N_ratio <- exp(((EP - EH) * (T1-T2))/(k*T1*T2))
N_ratio # 1.218111 for Early Spring
...
tegrespodryscale_mean <- tegrespodryscale %>%
  ...
  group_by(temp_treatment) %>%
  summarize(mean_Respo = mean(Respo, na.rm = TRUE))
R1 <- 1.104470 # metabolic rate at T1
R2 <- 1.888795 # metabolic rate at T2

EH <- - k * (log(R2/R1)) / (1/T2 - 1/T1)
EH  # 0.9798022 activation energy in eV
...
# create graph
abundanceratio <- data.frame(
  Experiment = c("Early Spring", "Late Spring"),
  EH = c(0.3943225, 0.9798022),
  EP = c(0.03404338, 0.1682466),
  Nratio = c(1.218111, 1.55961)
)

# Reshape for plotting EH and EP as bars
abundanceratio_long <- abundanceratio %>%
  dplyr::select(Experiment, EH, EP) %>%
  pivot_longer(cols = c(EH, EP), names_to = "Variable", values_to = "Value")

# Plot with dual axis: bars for EH/EP, points+line for Nratio
ggplot() +
  geom_col(data = abundanceratio_long,
           aes(x = Experiment, y = Value, fill = Variable),
           position = position_dodge(width = 0.7), width = 0.6) +
  geom_line(data = abundanceratio,
            aes(x = Experiment, y = Nratio, group = 1, color = "Nratio"),
            size = 1.2) +
  geom_point(data = abundanceratio,
             aes(x = Experiment, y = Nratio, color = "Nratio"),
             size = 3) +
  scale_fill_manual(values = c("EH" = "peachpuff3", "EP" = "seagreen"),
                    name = "Activation energy") +
  scale_color_manual(values = c("Nratio" = "black"),
                     name = "") +
  scale_y_continuous(
    name = "Activation energy (eV)",
    sec.axis = sec_axis(~ ., name = "N ratio") # mirror axis
  ) +
  labs(x = "Experiment") +
...
rwrespoplot + rwplot + tegrespoplot + tegcalcplot + plot_layout(guides = "collect", axes = "collect")
...
avgrwrespoplot + rwpredplot + avgtegrespoplot + avgcalcplot + plot_layout(guides = "collect", axes = "collect_x")
