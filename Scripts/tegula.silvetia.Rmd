...
# avg model may
avg_respo_model_may <- lm(avg_respo ~ pH * temp_treatment * predator, data = rwrespo_avg %>% filter(experiment == "b"))
...
```{r}
# Tegula respiration tables using rempsyc package
teg_respo_tables <- anova(avg_res_model_april, avg_res_model_may)
teg_respo_tables_tidy <- tidy(teg_respo_tables)

teg_respo_tables

teg_respo_april_table <- tidy(avg_res_model_april)
nice_table(teg_respo_april_table)

teg_respo_may_table <- tidy(avg_res_model_may)
nice_table(teg_respo_may_table)
