...
temp <- read.csv(here("Data","field","marine_ptfermin_temp.csv"))
```
...
```{r}
aprilpH <- aprilpH %>%
  mutate(experiment = "Low Upwelling")
maypH <- maypH %>%
  mutate(experiment = "High Upwelling")
AllpHData <- bind_rows(aprilpH, maypH)
...
AllpHData %>%
  ...
  labs(x = "Expected temperature", y = "Temperature in situ")
...
AllpHData %>%
  ...
  labs(x = "Expected pH", y = "pH in situ")
```
