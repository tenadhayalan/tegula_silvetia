...
temp <- read.csv(here("Data","field","ptfermin_temp_marine.csv"))
```
...
colnames(aprilPHavg) <- c("tank","pH","temp")
...
colnames(mayPHavg) <- c("tank","pH","temp")

# calculating avg pCO2 for april 
aprilcarbavg <- aggregate(cbind(pCO2) ~ tank, data = aprilcarb, FUN = mean)
aprilcarbavg$tank <- as.character(aprilcarbavg$tank)
colnames(aprilcarbavg) <- c("tank","pCO2")

# calculating avg pCO2 for may 
maycarbavg <- aggregate(cbind(pCO2) ~ tank, data = maycarb, FUN = mean)
maycarbavg$tank <- as.character(maycarbavg$tank)
colnames(maycarbavg) <- c("tank","pCO2")
...
```{r}
aprilpH <- aprilpH %>%
  mutate(experiment = "April") # assigning experimental month as April
maypH <- maypH %>%
  mutate(experiment = "May") # assigning experimental month as May

AllpHData <- bind_rows(aprilpH, maypH) #bind together

# temperature graph
AllpHData %>%
  ggplot(aes(x=as.factor(expected_temp), y= TempInSitu, group = interaction(expected_temp,experiment), color = experiment))+
  geom_boxplot(width = 0.5, outlier.alpha = 0.1)+
  scale_color_manual(values = c("#CF9EFD", "#8EF38E"))+
  theme_bw()+
  theme(strip.background = element_rect(fill = "white"),
        text = element_text(size = 16)) +
  labs(x = "Expected temperature", y = "Average tank temperature", color = "Experiment")

# pH graph
AllpHData %>%
  ggplot(aes(x=expected_pH, y= pH, group = interaction(expected_pH,experiment), color = experiment))+
  geom_boxplot(width=0.1, outlier.alpha = 0.1)+
  scale_color_manual(values = c("#CF9EFD", "#8EF38E"))+
  theme_bw()+
  theme(strip.background = element_rect(fill = "white"),
        text = element_text(size = 16)) +
  labs(x = "Expected pH", y = "Average tank pH", color = "Experiment")
```
...
temp <- temp %>%
  ...
  filter(year== 2024, month %in% 3:6)

temp %>%
  ggplot(aes(x = as.factor(date), y = mean)) +
  geom_smooth(aes(x = date, y = mean),method = "loess", color = "salmon4", size = 1)+
  geom_point(aes(x = date, y = mean), color = "salmon4", alpha = 0.1) +
...
```{r, echo=FALSE}
beuti <- beuti %>%
  filter(year== 2024, month %in% 3:6) # select only data from March to May 2024
...
aprilTeg$experiment <- "April"
mayTeg$experiment <- "May"
...
tegulaSizes %>%
 ...
 facet_wrap(~experiment)+
  geom_point(aes(color = as.factor(temp), fill = as.factor(temp)))+
  theme_bw() +
  theme(strip.background = element_rect(fill = "white"),
        text = element_text(size = 14)) +
  scale_fill_manual(values = c("skyblue2", "firebrick1")) +
...
std_coefs<-as_tibble(effectsize(tegmodel_res_april)) %>% # get the effect sizes for april
  mutate(month = "April") %>% # add the season
  left_join(tidy(tegmodel_res_april) %>%
              select(Parameter = term, p.value)) %>% # bring in the p-values from the summary of the model (which is now correct now that the values are standardized -- Note you STILL report the ANOVA results when talking about your significance in the model.  These p-values essentially tell you if the values are different or not from your intercept (essentiall 0))
  bind_rows(as_tibble(effectsize(tegmodel_res_may)) %>% # bind it to the same for May
              mutate(month = "May") %>%
              left_join(tidy(tegmodel_res_may) %>%
                          select(Parameter = term, p.value))) %>%
...
```{r}
calc_new <- calc %>%
  filter(deltaTA < 100) %>% # get rid of outlier
  mutate(ID = as.factor(ID)) %>%
...
std_coefs<-as_tibble(effectsize(tegmodel_cal_april)) %>% # get the effect sizes for april
  mutate(month = "April") %>% # add the month
...
rwmodelGP_aR_full <- lmer(GrossPhoto ~ pH * temp_treatment + (1 | tank/predator), 
                    data = rwrespo_clean %>% filter(experiment == "a"))
rwmodelGP_aR_reduced <- lmer(GrossPhoto ~ pH * temp_treatment + (1 | tank), 
                      data = rwrespo_clean %>% filter(experiment == "a"))
...
avgtegrespoplot <-
  avgtegrespoplot +
  scale_x_continuous(breaks = c(7.2, 7.6, 8.0))+
  labs(y = expression(atop(Respiration~rate~residuals,(mu*mol~O[2]~g^-1~hr^-1))))
...
ggsave("response.png", path=here("Output"), width = 10, height = 8)
```
